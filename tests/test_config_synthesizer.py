...
async def test_synthesizer_includes_failure_reasons_on_retry():
    """Failure reasons show up in the adjustment prompt on the second call."""
    synthesizer = ConfigSynthesizer(model="gpt-4o")
    synthesizer._client = make_mock_client(GOOD_CONFIG)
    ...
    # first call sets _last_prompt via signals path
    await synthesizer.synthesize(SIGNALS, failure_reasons=[])
    captured_messages.clear()

    # second call with failures should use the adjustment path
...
async def test_synthesizer_uses_signals_on_first_call():
    """First call incorporates environmental signals into the prompt."""
    synthesizer = ConfigSynthesizer(model="gpt-4o")
    synthesizer._client = make_mock_client(GOOD_CONFIG)
    ...
    combined = " ".join(m["content"] for m in captured_messages)
    assert "decompose queries" in combined
